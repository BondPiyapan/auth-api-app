...
const App = () => {
  ...
  const authContext = React.useMemo(() => ({
    signIn: async (foundUser) => {
      ...
      // setUserToken('fgkj');
      // setIsLoading(false);
      const userToken = foundUser.accessToken;
    ...
    //     console.log(e);
    //   }
    //   console.log('user getRetrieve: ', userName);
    //   dispatch({ type: 'RETRIEVE_TOKEN', token: userToken, id: userName });
    // },
    // getUser: () => {
    //   var userName = loginState
    //   return userName
    // },
  }), []);
  ...
  useEffect(() => {
    setTimeout(async () => {
      ...
      // console.log('user token: ', userToken);
      dispatch({ type: 'RETRIEVE_TOKEN', token: userToken, id: JSON.parse(userName) });
    }, 1000);
  }, []);
  ...
  if (loginState.isLoading) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <ActivityIndicator color={'#16193c'} size="large" />
      </View>
    );
  }
